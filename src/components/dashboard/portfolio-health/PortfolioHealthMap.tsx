...
const getInvestmentMarginData = (skusList: any[]): InvestmentMarginSku[] => {
  return skusList.map(s => {
    ...
    if (s.name === 'Herbal Shampoo') {
      ...
      rec = 'Booming demand requires low capital layout of $14M; yields a high return margin of 85%. Primary focus for budget boost.';
      ...
      rec = 'Snack leader with low freight overhead. Low investment of $18M yields 78% returns. Increase promotional layout.';
    } else if (s.name === 'BrandC Chips (Spicy)') {
      investment = 28; returnMargin = 68;
      rec = 'High local demand pull; low capital required ($28M) to yield 68% returns. Optimize distributor placement.';
    } else if (s.name === 'Coconut Water 330ml') {
      investment = 35; returnMargin = 81;
      rec = 'Organic category with high profit return of 81% against moderate $35M capital expansion. Secure convenience store placement.';
    } else if (s.name === 'Mango Fizz 500ml') {
      investment = 75; returnMargin = 70;
      rec = 'Market leader requires substantial launch budget ($75M) for regional campaigns. Returns a strong 70% margin.';
    } else if (s.name === 'Laundry Pods Premium') {
      investment = 82; returnMargin = 74;
      rec = 'Premium category needs automated line upgrades ($82M) but offers excellent 74% margins once scaled.';
    } else if (s.name === 'Dish Soap 1K') {
      investment = 60; returnMargin = 62;
      rec = 'Steady household demand. Scaling production requires $60M with solid 62% margins.';
      ...
      rec = 'Severe logistics bottleneck (35d lead time). Requires $85M for warehouse overrides with poor 15% margin yields.';
...
const VPCommandCenter: React.FC<{ 
  ...
  const ALL_APPROVALS = [
    ...
    { id: 'p_vp1', role: 'VP Product Management', type: 'Launch', title: 'Portfolio Roadmap: Authorize BrandF Soda 250ml slim-can product extension GTM charter — $4.5M', age: '2 days', urgency: 'high', done: false },
    { id: 'p_vp2', role: 'VP Product Management', type: 'CAPEX', title: 'CAPEX Allocation: Approve Q3 product packaging automation capital budget — $12.5M', age: '6 days', urgency: 'high', done: false },
  ...
  const ALL_DECISIONS = [
    ...
    { id: 'd_vp2', role: 'VP Product Management', icon: '📦', iconBg: 'rgba(59,130,246,0.1)', iconColor: '#3b82f6', title: 'Strategic Investment: Scale BrandF Water portfolio footprint to capture double-digit growth', sub: 'Category Leader · +12.4% YoY', stats: [['Net Sales', '$17.03M'], ['Margin', '40%'], ['Growth', '12.4% YoY'], ['Opportunity', 'Expand footprint']], done: false },
  ...
  const regions = [
    { name: 'APAC', rev: '$312M', pct: 94, delta: '+7.6%', up: true },
    { name: 'Americas', rev: '$228M', pct: 78, delta: '−5.0%', up: false },
    { name: 'EMEA', rev: '$311M', pct: 88, delta: '+2.0%', up: true },
  ];
      ...
      <div className="glass-card bg-acies-gray text-white py-5 px-6 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          ...
          <div className="flex flex-wrap lg:flex-nowrap gap-px shrink-0 bg-white/5 p-1 rounded-sm">
            {[
              { label: 'Safety Capital Released', value: '$42.2M', sub: 'Optimized tail reduction target' },
