  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        {/* LEFT COLUMN: REGIONS, BOTTLENECKS */}
        <div className="xl:col-span-3 space-y-6">
          ...
          <div className="glass-card bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 p-4 rounded-sm shadow-sm space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-black/5 dark:border-white/5">
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Region Performance</span>
            </div>
            <div className="space-y-3.5">
              {regions.map(r => (
                <div key={r.name} className="flex items-center justify-between text-xs">
                  <div className="min-w-0">
                    <p className="font-bold text-zinc-800 dark:text-zinc-200">{r.name}</p>
                    <p className="text-[9px] font-bold font-mono text-zinc-400 mt-0.5">{r.rev}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-16 h-1.5 bg-black/5 dark:bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${r.pct}%`, backgroundColor: r.up ? '#10b981' : '#ef4444' }} />
                    </div>
                    <span className="text-[10px] font-bold font-mono min-w-[36px] text-right" style={{ color: r.up ? '#10b981' : '#ef4444' }}>
                      {r.delta}
                    </span>
                  </div>
                </div>
        ...
        {/* CENTER COLUMN: MAIN CHARTS & EVENT FEED */}
        <div className="xl:col-span-6 space-y-6 flex flex-col">
          {/* Charts Grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 p-4 rounded-sm shadow-sm">
            {/* Revenue vs Target */}
            <div className="flex flex-col justify-between">
              <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-400 mb-3">Revenue vs Target — ₹ Cr</span>
                  ...
                  <LineChart 
                    ...
                    onClick={(data) => {
                      if (data && data.activeLabel) {
                        ...
                        addToast('Month Selected', `Reviewing details for ${data.activeLabel}.`, '#3b82f6');
                      }
                    }}
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke={gridStroke} />
                    <XAxis dataKey="name" tick={{ fill: tickColor, fontSize: 8 }} />
                    <YAxis tick={{ fill: tickColor, fontSize: 8 }} />
                    <Tooltip 
                    ...
                    <Line 
                      ...
                      strokeWidth={2} 
                      dot={(props) => {
                        const isSelected = selectedMonth === props.payload.name;
                        return (
                          <circle 
                            key={`dot-${props.index}`}
                            cx={props.cx} 
                            cy={props.cy} 
                            r={isSelected ? 4.5 : 1.5} 
                    ...
                    />
                    <Line type="monotone" dataKey="Target" stroke={tickColor} strokeDasharray="3 3" strokeWidth={1.2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-between items-center relative">
              <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-400 mb-3 self-start">Category Mix — MTD</span>
              <div className="h-[170px] w-full relative flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <RePieChart>
                    <Pie 
                      ...
                      paddingAngle={2}
                      onClick={(data, index) => {
                        const catName = categoryMixData[index].name;
                        setSelectedCategory(prev => prev === catName ? 'all' : catName);
                        addToast('Category Filter Applied', `Showing monthly trend and SKU health for ${catName} only.`, '#3b82f6');
                      }}
                    >
                      {categoryMixData.map((entry, index) => {
                        ...
                        return (
                          <Cell 
                            ...
                            opacity={selectedCategory === 'all' || isSelected ? 1 : 0.45}
                ...
                <div className="absolute text-center pointer-events-none">
                  <p className="text-[7px] font-bold uppercase tracking-wider text-zinc-450 dark:text-zinc-555 leading-none mb-0.5">Total</p>
                  <p className="text-xs font-display font-extrabold text-zinc-800 dark:text-zinc-200 leading-none">
                    {selectedCategory === 'all' ? '₹939Cr' : selectedCategory === 'Beverages' ? '₹316Cr' : selectedCategory === 'Snacks' ? '₹253Cr' : selectedCategory === 'Personal Care' ? '₹225Cr' : '₹145Cr'}
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* SKU Health Index */}
            <div className="flex flex-col justify-between">
              <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-400 mb-3">SKU Health Index</span>
              <div className="h-[170px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart 
                    data={activeSkuHealthData} 
                    layout="vertical" 
                    margin={{ left: -15, right: 5, top: 5, bottom: 5 }}
                    onClick={(state) => {
                      if (state && state.activeLabel) {
                        setSelectedHealthTier(prev => prev === state.activeLabel ? null : state.activeLabel);
                        addToast('Health Tier Selected', `Auditing ${state.activeLabel} SKUs.`, '#8b5cf6');
                      }
                    }}
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke={gridStroke} horizontal={false} />
                    <XAxis type="number" tick={{ fill: tickColor, fontSize: 8 }} />
                    <YAxis type="category" dataKey="name" tick={{ fill: tickColor, fontSize: 8 }} width={55} />
                    <Tooltip 
                      contentStyle={{ backgroundColor: tooltipBg, border: `1px solid ${tooltipBorder}`, color: tooltipText }}
                      itemStyle={{ fontSize: 9 }}
                    />
                    <Bar dataKey="count" radius={[0, 2, 2, 0]} barSize={8}>
                      {activeSkuHealthData.map((entry, index) => {
                        const isSelected = selectedHealthTier === entry.name;
                        return (
                          <Cell 
                            key={`cell-${index}`} 
                            fill={entry.color} 
                            opacity={selectedHealthTier === null || isSelected ? 1 : 0.45}
                            stroke={isSelected ? '#8b5cf6' : 'none'}
          ...
          {/* Interactive filter deck display and drills */}
          {(selectedCategory !== 'all' || selectedHealthTier !== null || selectedMonth !== null) && (
            <div className="p-4 bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-sm shadow-sm animate-fade-in flex flex-col gap-4">
              <div className="flex justify-between items-center pb-2 border-b border-black/5 dark:border-white/5">
                <div className="space-y-1">
                  <span className="text-[7.5px] font-black uppercase bg-[#8b5cf6]/20 text-[#8b5cf6] px-2 py-0.5 rounded-sm">INTERACTIVE DRILL PANEL ACTIVE</span>
                  <div className="flex flex-wrap gap-2 items-center text-[10px] text-zinc-800 dark:text-zinc-200 mt-1">
                    {selectedCategory !== 'all' && (
                      <span className="bg-blue-500/10 text-blue-500 border border-blue-500/20 px-2.5 py-0.5 rounded-sm font-semibold">
                        Category: <strong>{selectedCategory}</strong>
                      </span>
                    )}
                    {selectedHealthTier !== null && (
                      <span className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-2.5 py-0.5 rounded-sm font-semibold">
                        Health Tier: <strong>{selectedHealthTier}</strong>
                      </span>
                    )}
                    {selectedMonth !== null && (
                      <span className="bg-[#8b5cf6]/10 text-[#8b5cf6] border border-[#8b5cf6]/20 px-2.5 py-0.5 rounded-sm font-semibold">
                ...
                <button 
                  onClick={() => {
                    ...
                    setSelectedHealthTier(null);
                    setSelectedMonth(null);
                    addToast('Filters Cleared', 'Portfolio health command deck reset to portfolio views.', '#3b82f6');
                  }}
                  className="px-3.5 py-1.5 border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 text-[9px] font-bold uppercase rounded cursor-pointer text-zinc-550 dark:text-zinc-450 shrink-0 transition-colors"
                >
              ...
              {selectedMonth && (
                <div className="text-xs font-semibold space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">📊 Reconciled Stats for {selectedMonth}:</p>
                  <div className="p-3 bg-black/[0.01] dark:bg-white/[0.01] border border-black/5 dark:border-white/5 rounded-sm flex justify-between items-center gap-4">
                    <div className="space-y-0.5">
                      <p className="font-bold text-zinc-805 dark:text-zinc-200">
                        Monthly Revenue: <span className="text-[#3b82f6] font-extrabold">₹{selectedCategory === 'all' ? (monthlyRevenueData.find(d => d.name === selectedMonth)?.Actual || '866') : (activeRevenueData.find(d => d.name === selectedMonth)?.Actual || '290')} Cr</span>
                      </p>
                      <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 leading-snug font-medium">
                        Target for this period: ₹{selectedCategory === 'all' ? (monthlyRevenueData.find(d => d.name === selectedMonth)?.Target || '884') : (activeRevenueData.find(d => d.name === selectedMonth)?.Target || '297')} Cr.
                      </p>
                    </div>
                    <span className="px-2.5 py-1 text-[8.5px] font-bold uppercase bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-sm">
                      Reconciled ✓
                    </span>
                  </div>
                </div>
              ...
              {selectedHealthTier && (
                <div className="space-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">🔍 SKU Diagnostic Audit — {selectedHealthTier} Tier:</p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                    {selectedHealthTier === 'Healthy' && (
                      <>
                        <div className="p-2 border border-black/5 dark:border-white/5 rounded-sm bg-black/[0.005] dark:bg-white/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-800 dark:text-zinc-200">Mango Fizz 250ml</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Beverages · Margin 38% · Growth +12%</p>
                        </div>
                        <div className="p-2 border border-black/5 dark:border-white/5 rounded-sm bg-black/[0.005] dark:bg-white/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-800 dark:text-zinc-200">Herbal Shampoo Standard</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Personal Care · Margin 47% · Growth +28%</p>
                        </div>
                        <div className="p-2 border border-black/5 dark:border-white/5 rounded-sm bg-black/[0.005] dark:bg-white/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-800 dark:text-zinc-200">Oat Cookies Large</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Snacks · Margin 44% · Growth +15%</p>
                        </div>
                      </>
                    )}
                    {selectedHealthTier === 'At Risk' && (
                      <>
                        <div className="p-2 border border-red-500/10 rounded-sm bg-red-500/[0.02] text-[11px] font-semibold">
                          <p className="font-bold text-red-500">Fabric Softener Standard</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Household · Safety Stock Low · 7 Stockouts</p>
                        </div>
                        <div className="p-2 border border-amber-500/10 rounded-sm bg-amber-500/[0.02] text-[11px] font-semibold">
                          <p className="font-bold text-amber-500">Green Tea RTD 500ml</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Beverages · Margin declining QoQ · −1.1pp/mo</p>
                        </div>
                        <div className="p-2 border border-amber-500/10 rounded-sm bg-amber-500/[0.02] text-[11px] font-semibold">
                          <p className="font-bold text-amber-500">Foam Face Wash Care</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Personal Care · Safety margin compression · −1.0pp/mo</p>
                        </div>
                      </>
                    )}
                    {selectedHealthTier === 'Promo Dep.' && (
                      <>
                        <div className="p-2 border border-zinc-500/15 rounded-sm bg-black/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-700 dark:text-zinc-300">Choco Wafers Summer Deal</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Snacks · Promo dependence 72% · ROI −0.42x</p>
                        </div>
                        <div className="p-2 border border-zinc-500/15 rounded-sm bg-black/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-700 dark:text-zinc-300">Floor Cleaner Standard Bundle</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Household · Promo dependence 60% · ROI −0.18x</p>
                        </div>
                        <div className="p-2 border border-zinc-500/15 rounded-sm bg-black/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-700 dark:text-zinc-300">Green Tea RTD BOGO</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Beverages · Promo dependence 62% · ROI −0.62x</p>
                        </div>
                      </>
                    )}
                    {selectedHealthTier === 'Declining' && (
                      <>
                        <div className="p-2 border border-red-500/15 rounded-sm bg-red-500/[0.02] text-[11px] font-semibold">
                          <p className="font-bold text-red-500">Snacks Variant X</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Snacks · MoM margin decline −1.2pp/mo</p>
                        </div>
                        <div className="p-2 border border-red-500/15 rounded-sm bg-red-500/[0.02] text-[11px] font-semibold">
                          <p className="font-bold text-red-500">Green Tea RTD</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Beverages · MoM margin decline −1.1pp/mo</p>
                        </div>
                      </>
                    )}
                    {selectedHealthTier === 'Rationalize' && (
                      <>
                        <div className="p-2 border border-zinc-500/15 rounded-sm bg-black/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-800 dark:text-zinc-200">Floor Cleaner Standard</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Complexity index 0.81 · Annual Rev at risk ₹38Cr</p>
                        </div>
                        <div className="p-2 border border-zinc-500/15 rounded-sm bg-black/[0.005] text-[11px] font-semibold">
                          <p className="font-bold text-zinc-800 dark:text-zinc-200">Bitter Vanilla Extra</p>
                          <p className="text-[9.5px] text-zinc-450 dark:text-zinc-500 font-medium">Complexity index 0.74 · Margin 32% (Tail candidate)</p>
                        </div>
                      </>
                    )}
                  </div>
          ...
          <div className="glass-card bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 p-4 rounded-sm shadow-sm flex-1 flex flex-col gap-4">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 max-h-[360px] min-h-[300px]">
